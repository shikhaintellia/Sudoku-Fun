...
//     return null;
// };
export const generateHint = (board, selectedRow, selectedCol) => {
  // Check if the selected cell is empty
  if (board[selectedRow][selectedCol] === 0) {
    const possibleNumbers = getPossibleNumbers(selectedRow, selectedCol, board);
    if (possibleNumbers.length === 1) {
      return {
        cell: [selectedRow, selectedCol],
        number: possibleNumbers[0],
        message: `Hint: this cell must be ${possibleNumbers[0]}`
      };
    }
  }

  // Return null if no hint can be provided for the selected cell
  return null;
};
